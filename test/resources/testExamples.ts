import { Contract, Wallet, utils } from "ethers";

import { TestCaseInput } from "./models";
import { indefiniteOrder } from "./orderCreation";

export const baseTestInput = function (
  token0: Contract,
  token1: Contract,
  InteractionrsToken0: Wallet[],
  InteractionrsToken1: Wallet[],
): TestCaseInput {
  return {
    fundingAMMToken0: utils.parseEther("10"),
    fundingAMMToken1: utils.parseEther("10"),
    sellOrdersToken0: [
      indefiniteOrder(
        utils.parseEther("1"),
        utils.parseEther("0.9"),
        token0,
        token1,
        InteractionrsToken0[0],
        1,
      ),
    ],
    sellOrdersToken1: [
      indefiniteOrder(
        utils.parseEther("0.9"),
        utils.parseEther("0.90111"),
        token1,
        token0,
        InteractionrsToken1[0],
        1,
      ),
    ],
  };
};

export const fourOrderTestInput = function (
  token0: Contract,
  token1: Contract,
  InteractionrsToken0: Wallet[],
  InteractionrsToken1: Wallet[],
): TestCaseInput {
  return {
    fundingAMMToken0: utils.parseEther("10"),
    fundingAMMToken1: utils.parseEther("10"),
    sellOrdersToken0: [
      indefiniteOrder(
        utils.parseEther("1"),
        utils.parseEther("0.9"),
        token0,
        token1,
        InteractionrsToken0[0],
        1,
      ),
      indefiniteOrder(
        utils.parseEther("0.5"),
        utils.parseEther("0.45"),
        token0,
        token1,
        InteractionrsToken0[1],
        2,
      ),
    ],
    sellOrdersToken1: [
      indefiniteOrder(
        utils.parseEther("0.9"),
        utils.parseEther("0.90111"),
        token1,
        token0,
        InteractionrsToken1[0],
        1,
      ),
      indefiniteOrder(
        utils.parseEther("0.45"),
        utils.parseEther("0.45"),
        token1,
        token0,
        InteractionrsToken1[1],
        1,
      ),
    ],
  };
};

export const oneOrderSellingToken0IsOmittedTestInput = function (
  token0: Contract,
  token1: Contract,
  InteractionrsToken0: Wallet[],
  InteractionrsToken1: Wallet[],
): TestCaseInput {
  return {
    fundingAMMToken0: utils.parseEther("10"),
    fundingAMMToken1: utils.parseEther("10"),
    sellOrdersToken0: [
      indefiniteOrder(
        utils.parseEther("10.1"),
        utils.parseEther("9.7"),
        token0,
        token1,
        InteractionrsToken0[0],
        1,
      ),
      indefiniteOrder(
        utils.parseEther("5"), // <--- this order would can not be Interactiond fully against uniswap
        utils.parseEther("4.91"),
        token0,
        token1,
        InteractionrsToken0[1],
        2,
      ),
    ],
    sellOrdersToken1: [
      indefiniteOrder(
        utils.parseEther("10"),
        utils.parseEther("9"),
        token1,
        token0,
        InteractionrsToken1[0],
        1,
      ),
    ],
  };
};

export const noSolutionTestInput = function (
  token0: Contract,
  token1: Contract,
  InteractionrsToken0: Wallet[],
  InteractionrsToken1: Wallet[],
): TestCaseInput {
  return {
    fundingAMMToken0: utils.parseEther("10"),
    fundingAMMToken1: utils.parseEther("10"),
    sellOrdersToken0: [
      indefiniteOrder(
        utils.parseEther("10"),
        utils.parseEther("9.7"),
        token0,
        token1,
        InteractionrsToken0[0],
        1,
      ),
      indefiniteOrder(
        utils.parseEther("5"),
        utils.parseEther("4.9"),
        token0,
        token1,
        InteractionrsToken0[1],
        2,
      ),
    ],
    sellOrdersToken1: [
      indefiniteOrder(
        utils.parseEther("3"),
        utils.parseEther("2"),
        token1,
        token0,
        InteractionrsToken1[0],
        1,
      ),
      indefiniteOrder(
        utils.parseEther("3"),
        utils.parseEther("2.5"),
        token1,
        token0,
        InteractionrsToken1[1],
        2,
      ),
      indefiniteOrder(
        utils.parseEther("3"),
        utils.parseEther("2.6"),
        token1,
        token0,
        InteractionrsToken1[2],
        3,
      ),
    ],
  };
};

export const oneOrderSellingToken1IsOmittedTestInput = function (
  token0: Contract,
  token1: Contract,
  InteractionrsToken0: Wallet[],
  InteractionrsToken1: Wallet[],
): TestCaseInput {
  return {
    fundingAMMToken0: utils.parseEther("10"),
    fundingAMMToken1: utils.parseEther("10"),

    sellOrdersToken0: [
      indefiniteOrder(
        utils.parseEther("10"),
        utils.parseEther("9"),
        token1,
        token0,
        InteractionrsToken0[0],
        1,
      ),
      indefiniteOrder(
        utils.parseEther("3"),
        utils.parseEther("2.5"),
        token1,
        token0,
        InteractionrsToken0[1],
        2,
      ),
      indefiniteOrder(
        utils.parseEther("3"),
        utils.parseEther("2.5"),
        token1,
        token0,
        InteractionrsToken0[2],
        3,
      ),
      indefiniteOrder(
        utils.parseEther("3"), // <--- this order would can not be Interactiond fully against uniswap
        utils.parseEther("2.99"),
        token1,
        token0,
        InteractionrsToken0[3],
        4,
      ),
    ],
    sellOrdersToken1: [
      indefiniteOrder(
        utils.parseEther("10"),
        utils.parseEther("9.7"),
        token0,
        token1,
        InteractionrsToken1[0],
        1,
      ),
      indefiniteOrder(
        utils.parseEther("5"),
        utils.parseEther("4.9"),
        token0,
        token1,
        InteractionrsToken1[1],
        2,
      ),
    ],
  };
};

export const switchTokenTestInput = function (
  token0: Contract,
  token1: Contract,
  InteractionrsToken0: Wallet[],
  InteractionrsToken1: Wallet[],
): TestCaseInput {
  return {
    fundingAMMToken0: utils.parseEther("10"),
    fundingAMMToken1: utils.parseEther("10"),
    sellOrdersToken0: [
      indefiniteOrder(
        utils.parseEther("10"),
        utils.parseEther("9.7"),
        token0,
        token1,
        InteractionrsToken0[0],
        1,
      ),
      indefiniteOrder(
        utils.parseEther("5"),
        utils.parseEther("4.9"),
        token0,
        token1,
        InteractionrsToken0[1],
        2,
      ),
    ],
    sellOrdersToken1: [
      indefiniteOrder(
        utils.parseEther("10"),
        utils.parseEther("9"),
        token1,
        token0,
        InteractionrsToken1[0],
        1,
      ),
      indefiniteOrder(
        utils.parseEther("3"),
        utils.parseEther("2.5"),
        token1,
        token0,
        InteractionrsToken1[1],
        2,
      ),
      indefiniteOrder(
        utils.parseEther("3"),
        utils.parseEther("2.6"),
        token1,
        token0,
        InteractionrsToken1[2],
        3,
      ),
      indefiniteOrder(
        utils.parseEther("3"),
        utils.parseEther("2.9"),
        token1,
        token0,
        InteractionrsToken1[3],
        4,
      ),
    ],
  };
};
